...
use datafusion_physical_expr_common::{binary_map::{ArrowBytesMap, OutputType}, binary_map_v2::ArrowBytesMapV2};

/// A [`GroupValues`] storing single column of Utf8/LargeUtf8/Binary/LargeBinary values
///
/// This specialization is significantly faster than using the more general
/// purpose `Row`s format
pub struct GroupValuesMultiColumn<O: OffsetSizeTrait> {
    /// Map string/binary values to group index
    map: ArrowBytesMapV2<O, usize>,
    /// The total number of groups so far (used to assign group_index)
    num_groups: usize,
    schema: SchemaRef
}

impl<O: OffsetSizeTrait> GroupValuesMultiColumn<O> {
    pub fn new(schema: SchemaRef, output_type: Vec<OutputType>) -> Self {
        Self {
            map: ArrowBytesMapV2::new(output_type, schema),
            num_groups: 0,
            schema
        }
    }
...
impl<O: OffsetSizeTrait> GroupValues for GroupValuesMultiColumn<O> {
    fn intern(
        &mut self,
        cols: &[ArrayRef],
