...
mod tests {
    ...
    -> Result<()> {
        ...
        //
        // Forced hash collisions put every build row in one partition, and a
        // single non-empty partition needs no routing, so only the widening is
        // asserted in that configuration.
