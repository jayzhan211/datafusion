...
pub fn coerce_types(
    ...
    match agg_fun {
        ...
        AggregateFunction::Median | AggregateFunction::LastValue => {
            Ok(input_types.to_vec())
        }
...
pub fn check_arg_count(
    ...
    match signature {
        ...
        TypeSignature::OneOf(variants) => {
            let ok = variants
                .iter()
                .any(|v| check_arg_count(fun_name, input_types, v).is_ok());
            if !ok {
                return plan_err!(
        ...
        TypeSignature::VariadicAny => {
            if input_types.is_empty() {
                return plan_err!(
                    "The function {fun_name} expects at least one argument"
                );
            }
        }
        _ => {
